...
file key through KMS, decrypts into a stream, verifies magic bytes, scans the stream,
...
```text
Browser --TLS--> Cloudflare Worker --> private Container --> TLS PostgreSQL
                        |                 |       |
                        |                 |       +--> local ClamAV
...
chunks bypass the API only through narrowly scoped pre-signed storage operations.
...
| Stolen password | Mandatory TOTP, recovery-code rotation, generic login responses | Auth routes and DB transactions |
...
| Public or replayed download URL | Auth-bound, hashed, signed, 60-second, one-use ticket; attachment-only response | Download route transaction |
...
| Server/database compromise | Encrypted object bytes and wrapped DEKs; secrets in Cloudflare bindings; least-privilege TLS database role | Container isolation, bindings, PostgreSQL |
...
| A02 Cryptographic Failures | TLS/HSTS deployment requirement, AES-256-GCM, KMS envelope encryption, Argon2id, random opaque tokens; backup encryption is an operator control |
...
| A10 Server-Side Request Forgery | No user-controlled outbound URLs; S3/KMS/Resend/ClamAV endpoints come only from trusted configuration and should be egress-allowlisted |

## Test scope and limitations

Automated tests cover cryptography/tamper rejection, TOTP replay, filename/path and
magic-byte policy, request origin/CSRF boundaries, security headers, and authorization
query invariants. They do not replace a PostgreSQL-backed integration suite, real S3
multipart/CORS tests, real KMS/ClamAV/Resend staging tests, infrastructure review, or an
